...
class AsyncNodePluginTests: XCTestCase {
    ...
    func testContructionAsyncPluginWithoutHandler() {
        ...
        plugin.hooks?.onAsyncNode.tap({ node, callback in
            ...
            return replacementNode.handlerTypeToJSValue(context: self.context) ?? JSValue()
        })
    ...
    func testAsyncNodeWithAnotherAsyncNodeDelay() {
        ...
        let resolveHandler: AsyncHookHandler = { _,_ in
            ...
            sleep(3)
            return .singleNode(.concrete(self.context.evaluateScript("""
                    ([
    ...
    func testReplaceAsyncNodeWithChainedMultiNodes() {
        ...
        let resolve: AsyncHookHandler = { _,_ in
            ...
            if count == 1 {
                return .multiNode([
                    ReplacementNode.concrete(self.context.evaluateScript("""
                        (
            ...
            return .singleNode(ReplacementNode.concrete(self.context.evaluateScript("") ?? JSValue()))
        }
    ...
    func testHandleEmptyNode() {
        ...
        args = replacementResult.handlerTypeToJSValue(context: self.context)
    ...
    func testHandleMultipleUpdatesThroughCallback() {
        ...
        var replacementResult = AsyncNodeHandlerType.singleNode(.concrete(self.context.evaluateScript("""
                (
                    {"asset": {"id": "text", "type": "text", "value":"new node from the hook 2"}}
                )
            """) ?? JSValue()))
