...
const getNodeFromError = (
  ...
  if (playerError.errorType === ASYNC_ERROR_TYPE) {
    const { node } = playerError.metadata ?? {};
    if (typeof node === "object" && node !== null && !Array.isArray(node)) {
      const cacheEntry = context.asyncNodeCache.get((node as Node.Async).id);
      // Get first node if available. All will map back to the same async node and have the same parent so it doesn't matter which is picked here.
      return cacheEntry?.updateNodes.values().next().value;
    }
  }
