import { ErrorTypes, NodeType, getNodeID } from "@player-ui/player";
import type {
  Player,
  PlayerPlugin,
  Node,
...
type AsyncPluginContext = {
  ...
  /** Maps generated nodes to the async id that generated them. */
  oneMoreCache: Map<Node.Node, string>;
  ...
  /** map of async nodes ids to their async nodes. */
  sureWhyNotOneMoreThing: Map<string, Node.Async>;
};
...
export type ThingType = Resolve.NodeResolveOptions & {
  asyncData?: {
    generatedBy?: string;
  };
};

const isThingType = (thing: Resolve.NodeResolveOptions): thing is ThingType => {
  return true;
};
...
export class AsyncNodePluginPlugin implements AsyncNodeViewPlugin {
  ...
  applyResolver(resolver: Resolver, context: AsyncPluginContext): void {
    resolver.hooks.resolveOptions.tap(this.name, (options, node) => {
      if (!isThingType(options)) {
        return options;
      }

      const generatedArray = Array.isArray(options.asyncData?.generatedBy)
        ? options.asyncData?.generatedBy
        : [];

      // TODO: edge case - is not originally an async node, but becomes one through a transform or w/e

      // 1. If it is an async node, then it will be generated by it.
      if (this.isAsync(node)) {
        return {
          ...options,
          asyncData: {
            generatedBy: [node.id, ...generatedArray],
          },
        };
      }

      // 2. If it was generated through `resolveAsyncChildren` it should exist in oneMoreCache
      const thing = context.oneMoreCache.get(node);
      if (thing) {
        return {
          ...options,
          asyncData: {
            generatedBy: [thing, ...generatedArray],
          },
        };
      }
    ...
    player.hooks.errorController.tap("async", (errorController) => {
      errorController.hooks.onError.tap("async", (playerError) => {
        ...
        while (node !== undefined) {
          const generatedBy = currentContext.oneMoreCache.get(node);
          if (generatedBy) {
            const asyncNode =
              currentContext.sureWhyNotOneMoreThing.get(generatedBy);
            if (!asyncNode) {
              continue;
            }
    ...
    player.hooks.viewController.tap("async", (viewController) => {
      viewController.hooks.view.tap("async", (view) => {
        ...
        const context: AsyncPluginContext = {
          ...
          oneMoreCache: new Map(),
          assetIdCache: new Map(),
          sureWhyNotOneMoreThing: new Map(),
        };
