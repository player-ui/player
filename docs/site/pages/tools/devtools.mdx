...
In order for the devtools extension communicating with the player in your content; you must have a plugin that is using devtools-client. You may use our `BasicWevDevtoolsPlugin` as one the plugins in your Player UI and that will connect to the browser extension if it is installed.
