...
_Note: A ready-to-use DSLSchema Type is shipped with `@player-ui/reference-assets-components`. This schema is predefined with the data and validator references inferred from the `@player-ui/common-types-plugin`. Next, you'll be presented the steps in its creation for reference._

The first step to define your define your DSLSchema is importing the `DSLSchema` type and the relevant helper types and utilities from `@player-tools/dsl`. For this example we are importing the `common-types-plugin` in order to use its data types and validators. Our first step would be to generate the data type object types and references:
