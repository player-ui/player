...
_Note: The `Navigation` type we're importing here from the `@player-ui/types` package is different than the `Navigation` type from the `@player-tools/dsl` package. The former is the basic TypeScript definition for what Navigation in Player is. The latter  has specific replacements to take string/expression/react objects where normal objects would be defined._
