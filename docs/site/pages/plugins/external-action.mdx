...
In build.gradle
```kotlin
implementation "com.intuit.player.jvm.plugins:external-action:$JVM_PLAYER_VERSION"
```

In Player constructor
```kotlin
import com.intuit.player.plugins.externalAction.ExternalActionPlugin
...
```kotlin
// Plugin API
public fun onExternalAction(handler: ExternalActionHandler): Unit // Member function to configure external action handler
...
} // Functional interface for handling external state given the state, options controller and a transition function

// Player extensions
public val Player.externalActionPlugin: ExternalActionPlugin?
public fun Player.onExternalAction(handler: ExternalActionHandler): Unit
```
