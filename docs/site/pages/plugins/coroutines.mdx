...
```kotlin
// Plugin API
public val updates: ReceiveChannel<Asset?> // Public property that is used by the [UpdatesPlugin] to send [Asset] updates, can be used in a CoroutineContext to receive [Asset] updates.
public fun waitForUpdates(timeout: Long = 500): Asset? // This blocks the current thread until a new update is received or the timeout specified has passed.
...
```kotlin
// Plugin API
public var flowScope: CoroutineScope? // The CoroutineScope reflective of the current Player flow
public fun subScope(coroutineContext: CoroutineContext = Dispatchers.Default): CoroutineScope? // Build a child scope of the current flowScope to ensure that these scopes will be structured according to the current Flow

// Extension functions
public val CoroutineScope.flow: Flow? // Retrieves the current Player flow bound to the scope created by this plugin
