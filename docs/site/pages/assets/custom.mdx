...
```javascript
// Using WebAssetProvider from '@player-ui/asset-provider-plugin-react'
new WebAssetProvider([
...
// Used in the transform function for inputTranform
export interface TransformedDateInput extends InputAsset {
  ///  The `DataType` associated with this asset, for formatting the keyboard
  dataType?: DataType;
}
```
...
// select of TransformedChoiceA will have different implementation details than select of TransformedChoiceB
export interface TransformedChoiceA extends Choice {
  /** A function to commit the new value to the data-model */
  select: (newValue:  ValueType) =>  void;
}

export interface TransformedChoiceB extends Choice {
  /** A function to commit the new value to the data-model */
  select: (newValue:  ValueType) =>  void;
}
```

Then in the web transform registration choiceA and choiceB are registered to different transforms
