...
```
Since the transform is called on "select" of the wrapped function in the data this doesnt change the ChoiceData which means they can both be registered to ChoiceAsset

For example in the web transform registration choiceA and choiceB are registered to different transforms
