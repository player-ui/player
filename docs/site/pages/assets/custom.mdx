...
3. [Registering assets](#registering-assets)
...
#### Styling
While rendering a `ComposableAsset` within another asset, you might need to pass down styling. You can do so by passing the `Style` object while calling `compose`.
```
OutlinedTextField(
    // ... other congig
    label = { data.label.compose(
        Style(
            xmlStyles = listOf(R.style.Center),
            composeTextStyle = TextStyle(textAlign = TextAlign.Center)
        ))
    }
)
```
