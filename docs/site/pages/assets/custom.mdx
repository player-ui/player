...
```javascript
class CustomAssetPlugin implements WebPlayerPlugin{
  applyWeb(webplayer) {
...
Use the Asset from the `@player-ui/react-asset` package with the nested asset as props to dynamically determine the rendering implementation to use:
