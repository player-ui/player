---
title: External State View Modifier
platform: ios
---
...
<SPM product="ExternalStateViewModifierPlugin" />
...
```swift
let plugin = ExternalStateViewModifierPlugin<ExternalStateSheetModifier> { state, options, transition in
		return AnyView(
      VStack {
        Text("External State")
