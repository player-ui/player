...
The data stays queryable and editable through normal Player mechanisms. The plugin registers expressions that operate on the in-memory store and re-resolve any views reading the binding:
...
// Update a single record by a key field. The fourth argument is an object of fields to merge.
// Typically evaluated from an action's exp with a model-ref for the row key:
updateStreamingDataRecord("transactions", "id", {{transactions._row_.id}}, {"status": "reviewed"});
```
...
The same operations are available on the plugin instance for custom asset code that holds a plugin reference: `plugin.sort()`, `plugin.search()`, `plugin.clearQuery()`, and `plugin.update(binding, predicate, patch)`. `plugin.update()` accepts any predicate function and a partial-record patch object (or a function from the current record to a patch), giving you full control when the expression form is too rigid.

### Freeze behavior and update performance

By default records are frozen (`freeze` is `true`). When `updateStreamingDataRecord` or `plugin.update()` matches a record, it creates a new frozen copy with the merged fields. This rebuilds the view array, so every consumer gets a new reference — correct, but a full re-render at 50k rows.

Set `freeze: false` on the source config when you need in-place edits that don't rebuild the view:
...
With `freeze: false` and no active sort or search, `plugin.update()` mutates the matching record in place and only increments the revision counter without rebuilding the view array. The view reference is unchanged, so a memoized component won't re-render. Player is still notified (the status write triggers a dirty check), so the expression system and data controller see the change. If a sort or search is active when the update runs, the view is always rebuilt so the updated record lands in the right position.
...
- The streamed binding is **read-only through normal data-model writes**. Use `updateStreamingDataRecord` or `plugin.update()` to edit records — direct `set`/`delete` calls on the binding are warned-and-dropped
