...
Generate a JSON file of metrics using `@player-lang/metrics-output-plugin`. The plugin consumes diagnostics from LSP plugins and produces a single JSON file with per-file metrics and optional project-level metadata.

## Usage

Install the plugin:

<PackageManagers pkg="@player-lang/metrics-output-plugin" />
...
```js
[
  "@player-lang/metrics-output-plugin",
  {
...
import {
  ...
  extractByData,
} from "@player-lang/metrics-output-plugin";
