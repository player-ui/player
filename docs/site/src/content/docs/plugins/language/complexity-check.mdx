...
Generate diagnostics for content complexity using `@player-lang/complexity-check-plugin`. The plugin analyzes your Player Content and calculates complexity based on asset types, nesting depth, and overall structure to provide a single complexity score per file.

## Usage

Install the plugin:

<PackageManagers pkg="@player-lang/complexity-check-plugin" />
