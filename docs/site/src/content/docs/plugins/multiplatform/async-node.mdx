...
const asyncHandler = (node: Node.Async, callback: (content: any) => void) => {
  // do some async task to get content
  return result;
};
...
asyncNodePlugin.hooks.onAsyncNodeError.tap(
  "handleAsyncError",
  (error, node) => {
    // Check that the error can be recovered from for this node.
    if (!isRecoverable(error, node)) {
      return undefined;
    }

    // Create new content to be shown to the user.
    const newContent = getErrorPlaceholder(error, node);
    return newContent;
  },
);
```

<SPM product="AsyncNodePlugin" />
