...
The `@player-ui/partial-match-registry` package provides a `Registry` that functions as a fuzzy-match dictionary. It's used by this plugin and others like the [External State Plugin](/plugins/multiplatform/external-state/).
