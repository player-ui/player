...
player.start(a2uiSnapshot, { format: "a2ui" });
```
...
Wiring up the base Player plus the A2UI plugin (and, on native platforms, the renderer catalog) is the same handful of lines on every platform. In order to make it as easy as possible to get started with Player and A2UI, we offer "prepackaged" version of player on each platform with all the necessary plugins for A2UI rendering included off the shelf. These offerings are similarly shaped to the underlying Player packages but may have a couple more properties (when required) to handle some of the underlying wiring automatically. 
