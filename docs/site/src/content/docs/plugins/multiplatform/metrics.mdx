...
The `JVM` Metrics plugin tracks render time and update time for views in a flow. Both default to `true`, but can be disabled individually. On Android, `PlayerFragment` calls `renderEnd()` automatically once a view (or an update to it) finishes painting, so no host wiring is required. Hosts that render outside of `PlayerFragment` must call `renderEnd()` themselves after each paint.
...
Render and update tracking can be toggled independently:

```kotlin
val metricsPlugin = MetricsPlugin(trackRenderTime = true, trackUpdateTime = false) { timing, renderMetrics, flowMetrics ->
  ...
}
```
