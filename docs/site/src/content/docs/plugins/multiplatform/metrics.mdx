...
The `JVM` Metrics plugin tracks render time and update time for views in a flow (both always enabled, like the web wrapper). On Android, `PlayerFragment` calls `renderEnd()` automatically once a view (or an update to it) finishes painting, so no host wiring is required. Hosts that render outside of `PlayerFragment` must call `renderEnd()` themselves after each paint.
