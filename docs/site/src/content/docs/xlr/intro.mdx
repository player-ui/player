...
The Player LSP uses XLR to validate JSON content as you type, and the functional DSL generator uses it to produce type-safe content-authoring builders. See [Real-World Consumers](../architecture/consumers) for how both work.
...
If you are developing Player capabilities for Core/React, all you need to do is have your Plugin fill in the `ExtendedPlayerPlugin` interface and run the Player CLI during build time and the CLI will take care of compiling and bundling the capabilities. You can see an example of this in the [Exporting Plugin Capabilities](../usage/export#exporting-plugin-capabilities) section.

## As Someone Building Capabilities Around Player

If you want to leverage XLRs to develop capabilities around Player, you'll want to start with the [SDK](../usage/sdk) and use it to load the XLRs from published packages. Then, using the SDK, you can access all the loaded types and information about them for use in your application. 

## How the Pieces Fit Together

At a high level, XLR is a compile-time step followed by a runtime consumption step: TypeScript source is compiled into XLR JSON once, and that JSON can then be loaded by any number of consumers without ever touching a TypeScript compiler again.
...
For the full picture, including the package breakdown, the AST shape, and concrete consumers, see [Architecture](../architecture/overview).