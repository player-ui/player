...
> [!NOTE]
> The package follows a naming convention of `@player-ui/{description}-plugin-{platform}`.
...
> [!NOTE]
> - **External dependencies** are specified as `//:node_modules/...` with the leading `//`
> - **Internal dependencies** are specified as `:node_modules/...` without the leading `//`
...
bazel build //plugins/example-plugin/core
...
bazel test //plugins/example-plugin/core:test
...
For rapid development, you can use watch mode:

```bash
# Watch and rebuild on changes
bazel run //plugins/example-plugin/core:dev
```
...
- **Keep core logic platform-agnostic** in core plugins
- **Separate React-specific code** into React plugins
...
- **Use `vitest`** for consistent testing across the project
- **Mock external dependencies** appropriately

### Performance Considerations

- **Tree-shake unused code** using proper ES module exports
- **Minimize bundle size** for React plugins
...
```typescript
// Using hooks for dependency injection
export const useExamplePlugin = () => {
  const player = usePlayer();
  return player.getPlugin('example-plugin');
};
```

## Troubleshooting

For common Core & Web development issues, see the [Troubleshooting Guide](/contributing/troubleshooting).

## Related Resources

- [Main Contributing Guide](/contributing/) - General contributing guidelines
- [Troubleshooting Guide](/contributing/troubleshooting/) - Solutions to common development issues
