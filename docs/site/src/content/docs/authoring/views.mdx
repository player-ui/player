...
Templates are included via the `@player-lang/react-dsl` package. This can be used in any asset slot:

```tsx
import React from 'react';
import { dataTypes } from '@player-ui/common-types-plugin';
import { makeBindingsForObject, Template } from '@player-lang/react-dsl';
...
The `@player-lang/react-dsl` module also includes support for _static_ and _dynamic_ switches.

Use the `isDynamic` flag to denote this should be a `dynamicSwitch` instead of a `staticSwitch`:

```tsx
import React from 'react';
import { Switch } from '@player-lang/react-dsl';
