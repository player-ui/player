...
The `PlayerLanguageService` is the foundation of all Player Content. It conforms to the Language Server Protocol standards, and the docs refer to this `PlayerLanguageService` as The LSP. The LSP parses JSON to a manipulable Abstract Syntax Tree (AST). It also provides features like Content validation.

As a user of Player, you will not create your own LSP. The [Player CLI](../capabilities/cli) instantiates an LSP for you. Plugins that manipulate the LSP can be added to the Player CLI (i.e. DSL / CLI plugins). How to create and use DSL / CLI plugins is described in the [Plugins](./plugins) section.

You may interact with an LSP (via a DSL / CLI plugin) in the following ways:
...
These are functions that are part of the LSP standard. They provide useful features that may be leveraged by the Player CLI, or that could be used in an IDE setting (like autocomplete, find definition, etc.), but are not typically relevant for DSL / CLI plugin authors.
...
- If the path does NOT have `"types"` in the name, some default filters and transforms will be applied. They are defined [here](https://github.com/player-ui/tools/blob/ee81bb095147cf935f42cf7670b057086505db55/language/json-language-service/src/constants.ts#L29-L38).
...
If the manifest has a "Types" capability, some default filters and transforms will be applied. They are defined [here](https://github.com/player-ui/tools/blob/ee81bb095147cf935f42cf7670b057086505db55/language/json-language-service/src/constants.ts#L29-L38). If the manifest does NOT have a "Types" capability, no filters or transforms will be applied.
...
These are functions that are part of the LSP standard. They provide useful features that are leveraged by the Player CLI, or that can be used for IDE tooling (not yet used by the Player), but are not typically relevant for DSL / CLI plugin authors.
