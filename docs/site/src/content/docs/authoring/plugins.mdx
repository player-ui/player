...
Much like the rest of Player, Player's Domain Specific Language (DSL) compilation supports plugins that influence how DSL Content gets compiled into JSON that Player ingests. A DSL Plugin is a type of [CLI Plugin](../../capabilities/cli#plugins) that uses either:

1. [The hooks available on the CLI itself](#cli-hooks)
2. [The hooks available on the DSL compiler instance created by the CLI](#dsl-compiler-hooks)

Unlike plugins for the Player, DSL plugins are not implemented per platform. DSL plugins are implemented only once.

## Create a DSL Plugin

To create a DSL Plugin, create a class that implements `PlayerCLIPlugin`. Example:
...
`onCreateLanguageService` is called right after the [LSP](lsp) is created, before any validation or compilation has occurred. It allows you to tap into the [LSP's hooks](lsp#hooks-to-extend-lsp-functionality).
...
1. `exp` (`boolean`): A boolean indicating whether the LSP is in "experimental" mode. This flag has no inherent effects. It is entirely up to the user to decide what "experimental" means in their specific use case.
...
`onCreateDSLCompiler` is called right after the DSL compiler instance is created, before any compilation has occurred. It allows you to tap into the [DSL compiler's hooks](#dsl-compiler-hooks) to modify how DSL content is compiled.
...
`createCompilerContext` is called to expose hooks that influence how content is compiled. It provides access to the `CompilationContext` instance. The `CompilationContext` manages the context around DSL compilation and exposes [CLI hooks](#cli-hooks) for customizing the compilation process.
...
2. Create a [CLI config file](../../capabilities/cli/#config) in the root of your content project. The Player CLI uses [cosmiconfig](https://github.com/cosmiconfig/cosmiconfig) to automatically discover configuration files. See the [CLI documentation](../../capabilities/cli/#config) for a complete list of supported config file names and formats.

3. Configure your plugin in the config file. The format depends on whether you're using a [JavaScript/TypeScript file](#javascripttypescript-config-files) or a [JSON/YAML file](#jsonyaml-config-files). Instructions for each format are below.

4. Use the Player CLI commands. The CLI will automatically discover and load your config file.

    If you need to use a config file in a non-standard location, you can specify it with the `--config` flag:

    ```bash
    player dsl compile --config path/to/custom-config.json
    ```
