...
Much like the rest of Player, Player's Domain Specific Language (DSL) compilation supports plugins that influence how DSL Content gets compiled into JSON Content. A DSL Plugin is a type of [CLI Plugin](../capabilities/cli#plugins) that uses either:
1. The hooks available on the CLI itself
2. The hooks available on the DSL compiler instance created by the CLI.

Unlike plugins for the Player, DSL plugins are not implemented per platform. DSL plugins are implemented only once in JSX or TSX.
