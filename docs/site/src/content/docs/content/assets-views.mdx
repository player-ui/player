...
When writing template content, the position of the template in relation to non-template content is not guaranteed when it gets transformed into its final JSON output. Because of this, templates support a `placement` property that specifies whether the template should expand before or after pre-existing content.
...
- Place the expanded static template first, since it has `placement: prepend`
- Then display content from the `values` array
- Finally place the expanded dynamic template last, since it has `placement: append`
