...
The `schema` section of the content describes the relationship between the `view` and the `data`. It breaks down properties of the model's object into data-types, each with their own spot for validation, formatting, or default-values. This page covers what you author; for how it's parsed and served at runtime, see [SchemaController](/architecture/systems/schema-controller).
