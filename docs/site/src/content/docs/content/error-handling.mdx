...
The built-in error classes (`AssetRenderError`, `AsyncNodeError`, `ResolverError`, `ExternalStateError`) all implement this interface. For custom errors, extend `Error` and implement `PlayerErrorMetadata`:
