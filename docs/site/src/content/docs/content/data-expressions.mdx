...
For how bindings are parsed and resolved at runtime, see [BindingParser](/architecture/systems/binding-parser).

## Expressions

Expressions are callable functions that allow for dynamic behavior of Player and it's views. For the runtime engine that parses and evaluates them, see [ExpressionEvaluator](/architecture/systems/expression-evaluator).
