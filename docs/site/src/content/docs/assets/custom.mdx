...
1. [Extending DecodableAsset](#extending-decodableasset)
   ...
   4. [Styling](#styling)
2. [Extending ComposableAsset](#extending-composableasset) - Experimental
...
### Extending ComposableAsset (Experimental)


`ComposableAsset` is a subclass of `SuspendableAsset` designed for use with Jetpack Compose, Google's modern toolkit for building native UIs for Android. Subclassing `ComposableAsset` requires passing a `KSerializer<Data>` for the data class that represents the data for that asset. It also requires the implementation of an abstract `content` composable function which defines how the asset should be rendered with the data provided.
...
#### Styling
While rendering a `ComposableAsset` within another asset, you might need to pass down styling. You can do so by passing the `LocalTextStyle`.
```
...
In cases, where you have to render an Android view asset within a Compose asset, you can pass in a `modifier` and styles to the `AndroidView` as follows:
```
data.label?.compose(
