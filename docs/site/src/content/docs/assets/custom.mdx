...
#### Implementing `initView(data: Data)`

```kotlin
suspend fun initView(data: Data): View
```
...
#### Implementing `hydrate(data: Data)`

```kotlin
suspend fun View.hydrate(data: Data)
```
...
Both `initView` as well as `hydrate` are suspend functions by design. This is due to the blocking nature of Javascript execution, making these suspend functions enables non-blocking thread JS execution.

#### Accessing Data

In most cases, there is some additional data that is used to make the rendering more meaningful. For instance, the intent of the previous `text` asset example was to render a View that displayed the string contained in `value`. The data is provided as an argument to the above functions. Accessing data outside of the above methods should be done through the suspendable `getData` method to ensure data decoding is able to suspend instead of block the calling thread. This `data` returned is a type specified when defining the subclass.
...
```kotlin
 @Composable
override fun content(data: Data, modifier: Modifier) {
    Text(
        text = data.value,
