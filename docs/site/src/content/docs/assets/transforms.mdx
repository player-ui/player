...
Player will do minimal processing of each asset in the tree, resolving [data in strings](../../content/data-expressions), [applicability](../../content/assets-views#applicability) and any other generic processing it can. The processed tree is then sent to the rendering layer, where it is turned into a native component on the system Player is running on.
