...
### (iOS) `CommonExpressionsPlugin`, `ComputedPropertiesPlugin`, and `StageRevertDataPlugin` removed

These plugins are now **core-only** and no longer have iOS-specific implementations. They operate at the shared JS core layer and do not need to be registered per-platform.
