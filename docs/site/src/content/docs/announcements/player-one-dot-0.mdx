...
The plugin leverages the new `ErrorController` to throw an `ExternalStateError` (`type: "EXTERNAL-STATE"`) instead of silently stalling the flow. Flows can recover via `errorTransitions`. These are the cases that will trigger an error:
1. When no handler matches (`metadata.reason: "missing-handler"`)
2. When a handler returns no transition value (`metadata.reason: "missing-transition-value"`).

See the [Error Handling docs](/content/error-handling) for more on recovering from errors.
