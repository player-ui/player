...
The `clearExceptionHandler()` method has been removed from `SwiftUIPlayer`. Its cleanup is now handled automatically inside `unload()`, so calling it directly is no longer necessary.

#### Migration

Remove any calls to `clearExceptionHandler()`. The exception handler is cleared as part of `unload()`, which is invoked automatically when `ManagedPlayer` tears down.

```swift
// Before
player.clearExceptionHandler()

// After — no replacement needed; unload() handles it
```
