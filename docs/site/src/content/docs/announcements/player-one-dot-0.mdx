...
Instead, register these plugins once in the shared core layer of your Player plugin, following the process described in the
corresponding docs:

- [Common Types Plugin](../../plugins/core/common-types)
- [Common Expressions Plugin](../../plugins/core/common-expressions)
...
The plugin now reports an `ExternalStateError` (`type: "externalState"`) through the new
`ErrorController` instead of silently stalling the flow. Two failure modes:

1. No handler matches (`metadata.reason: "missing-handler"`)
2. A handler ran but returned no transition value (`metadata.reason: "missing-transition-value"`)

<Aside type="caution" title="Behavior change">
Any falsy return value from a handler (`undefined`, `""`, `0`, `null`) now triggers
`missing-transition-value`. Previously only `undefined` was treated as "no transition";
falsy values that used to silently stall the flow will now be reported.
</Aside>

Flows can recover via `errorTransitions` or by tapping `errorController.hooks.onError`.
See the [Error Handling docs](/content/error-handling).
...
### (React) `onStartedFlow` receives the started flow

The `ManagedPlayer` `onStartedFlow` callback now receives the `Flow` that was started, so consumers no longer need to track it separately.
...
**Migration:** No change required — the parameter is additive. Update your handler signature if you want access to the started flow.
...
See the [Error Handling docs](/content/error-handling) for the full API reference.
