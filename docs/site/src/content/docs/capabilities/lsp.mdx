...
As a user of Player, you will typically not create your own LSP, rather you'll use one that already integrated into a tool. For example, the [Player CLI](../cli) instantiates an LSP for use with content based operations. 

Plugins that manipulate the LSP can be added to the Player CLI. How to create and use CLI plugins is described in the [CLI Plugins](../cli#plugins) section.
...
The `LSPPluginPlugin` can be applied to the [Player CLI](../cli#lsppluginplugin) config to achieve the same functionality.
...
The `LSPTransformsPlugin` can be applied to the [Player CLI](../cli#lsptransformsplugin) config to achieve the same functionality.
