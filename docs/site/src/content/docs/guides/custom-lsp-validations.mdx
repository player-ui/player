...
LSP validation runs statically over authored content in an editor/build context. It's a separate system from the [ValidationController](/architecture/systems/validation-controller), which validates live user data against schema and cross-field rules at runtime while a flow is executing.
:::
...
import { getNodeValue } from '@player-lang/json-language-service';
...
import { LSPPluginPlugin, LSPAssetsPlugin } from "@player-cli/cli";
