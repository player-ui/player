...
```typescript
export class ExamplePlayerPlugin implements PlayerPlugin {
...
**Note**: `JSBasePlugin` implementations do not necessarily need to be a `PlayerPlugin`, for example, the [BeaconPlugin](https://github.com/player-ui/player/blob/main/plugins/beacon/apple/Sources/BaseBeaconPlugin.swift#L59) can take plugins in it's constructor, that are not `PlayerPlugin`.
...
### Rendering Alternative Components

Sometimes you may want to replace the entire Player component with an alternative view based on some external event or state. The `ReactPlayer` exposes a `viewUpdateSubscription` that can be used to trigger updates and pass data to your plugin.

Here is an example of how to use `viewUpdateSubscription` to render an alternative component:

```tsx
import { ReactPlayer, ReactPlayerPlugin } from "@player-ui/react";
import { useSubscribedState } from "@player-ui/react-subscribe";

export class AlternativeComponentPlugin implements ReactPlayerPlugin {
  name = "alternative-component-plugin";

  applyReact(rp: ReactPlayer) {
    rp.hooks.webComponent.tap(this.name, (Comp) => {
      return () => {
        const view = useSubscribedState(rp.viewUpdateSubscription);

        // Alternatively, without a hook:
        // rp.viewUpdateSubscription.get().type === "alternative-component"

        if (view?.type === "alternative-component") {
          return <div>Alternative Component</div>;
        }
...
const player = new ReactPlayer({
  plugins: [new AlternativeComponentPlugin()],
});

// Trigger the alternative component
player.viewUpdateSubscription.publish({ type: "alternative-component" });
```
