...
:::note[A different system from runtime validation]
This is a static, build/edit-time check of authored content shape via XLRs — it runs before content is published. It's a separate system from the [ValidationController](/architecture/systems/validation-controller), which validates live user data against schema and cross-field rules while a flow is running.
:::
...
The first step in validating content is to add your CLI preset as a dependency to the repository/project where the content lives. You'll also want to add the `@player-cli/cli`, `react`, and `@types/react` packages as a dependencies which will also pull in the underlying dependencies and make the tooling available to run. 
