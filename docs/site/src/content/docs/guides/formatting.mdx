...
At runtime, formatters are looked up through the [SchemaController](/architecture/systems/schema-controller) and applied via the `format`/`deformat` hooks on the [DataController](/architecture/systems/data-controller).

## Using Formatted Data

In order to get or set formatted values from the model, add `formatted: true` to the options of the data-model method. This is most often through the use of a [transform](/plugins/core/asset-transform).
