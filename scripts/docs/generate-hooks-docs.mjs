...
// Prefer Bazel-provided workspace root when available (genrules often run from execroot).
const repoRoot = process.env.BUILD_WORKSPACE_DIRECTORY
  ? path.resolve(process.env.BUILD_WORKSPACE_DIRECTORY)
...
function renderMDX(hooks) {
  ...
  lines.push('update: "bazel build //docs/site:site"');
  lines.push("---");
  lines.push("");
  lines.push(
