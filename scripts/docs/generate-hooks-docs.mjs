...
function parseArgs(argv) {
  const args = { roots: [], out: DEFAULT_OUT, check: false };
...
async function main() {
  ...
  if (args.help) {
    console.log(`Usage:
  node scripts/docs/generate-hooks-docs.mjs [--roots <dir>]... [--out <file>] [--check]
...
`);
  ...
  const patterns = args.roots.flatMap((r) => [
    path.posix.join(r.replaceAll("\\", "/"), "**/*.ts"),
    "!" + path.posix.join(r.replaceAll("\\", "/"), "**/__tests__/**"),
    "!" + path.posix.join(r.replaceAll("\\", "/"), "**/*.test.ts"),
    "!" + path.posix.join(r.replaceAll("\\", "/"), "**/*.spec.ts"),
    "!" + path.posix.join(r.replaceAll("\\", "/"), "**/node_modules/**"),
  ]);

  const absPatterns = patterns.map((p) =>
    p.startsWith("!")
      ? "!" + path.join(repoRoot, p.slice(1))
      : path.join(repoRoot, p),
  );

  const files = (await globby(absPatterns, { dot: false })).sort();
