...
export interface PlayerRuntimeConfig {
  /**
   * Up to what severity of logs generated by Player should be handled.
   */
  logLevel: Severity;

  /**
   * How strictly to parse expressions
   * - `off` Player will attempt to execute any expression regardless of if it syntactically/symantically correct and fail silently if there are issues
   * - `warn` Player will log a warning if there are syntax issues or the expression can not be executed symantically correct (e.g the use of a non-existant expression) but still attempt to execute it
   * - 'error` Player will throw an error if there is any syntax/symantic issues with the expression
   */
  expressionStrictness: "off" | "warn" | "error";

  /**
   * How strict Player's view resolution stage is about enforcing duplicate IDs
   * - `off` Player will not perform any checks on IDs for uniqueness
   * - `warn` Player will warn if there are assets/views with duplicate IDs but attempt to render the view
   * - `error` Player will throw an error if there are duplicate IDs
   */
  duplicateIDStrictness: "off" | "warn" | "error";
}
