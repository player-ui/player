...
describe("templates", () => {
  ...
  it("removes the templateSymbol after sorting", () => {
    // Create a mock parser
    const parser = createMockParser();

    // Create the plugin with mock options
    const options = createMockOptions();
    const templatePlugin = new TemplatePlugin(options);

    // Apply the plugin to the parser
    templatePlugin.applyParser(parser);
    ...
    // Mock the data for the template
    options.data.model.get = jest.fn().mockReturnValue([{ id: 1 }, { id: 2 }]);

    // Parse the template node to get a MultiNode
    const multiNode = templatePlugin["parseTemplate"](
      parser.parseObject.bind(parser),
      templateNode,
      options,
    );

    // Verify the MultiNode has the templateSymbol
    expect(multiNode).not.toBeNull();
    expect(multiNode?.type).toBe(NodeType.MultiNode);

    // Access the private templateSymbol
    // We need to get the actual symbol reference from the class
    const templateSymbol = Object.getOwnPropertySymbols(
      multiNode as object,
    ).find((sym) => sym.toString().includes("template"));

    expect(templateSymbol).toBeDefined();
    expect((multiNode as any)[templateSymbol as symbol]).toBe("append");

    // Create a parent node with children that include our MultiNode
    const parentNode = {
      type: NodeType.View,
      children: [
        {
    ...
    // Call the onCreateASTNode hook that contains the sorting and cleanup logic
    const result = parser.hooks.onCreateASTNode.call(parentNode);

    // Verify the templateSymbol has been removed
    expect(
      Object.getOwnPropertySymbols(multiNode as object).find((sym) =>
        sym.toString().includes("template"),
      ),
    ).toBeUndefined();

    // Alternative check using the symbol reference we captured earlier
    expect((multiNode as any)[templateSymbol as symbol]).toBeUndefined();
  });
