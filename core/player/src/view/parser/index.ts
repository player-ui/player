...
import { getNodeID, hasAsync, hasTemplateValues } from './utils';
...
export class Parser {
  public readonly hooks = {
    ...
    parseAndCreateChildNode: new SyncBailHook<
      [
        parseOptions: ParseObjectOptions,
        localKey: string,
        localValue: any,
    ...
    const parseLocalObject = (
      ...
      const newValue = objEntries.reduce((accumulation, current): NestedObj => {
        ...
        if (localKey === 'asset' && typeof localValue === 'object') {
          ...
          const newChildren = this.hooks.parseAndCreateChildNode.call(
            options,
            localKey,
            localValue,
            path
