...
internal sealed class AbstractJSIValueDecoder(
    ...
    override fun beginStructure(descriptor: SerialDescriptor): CompositeDecoder = runtime.evaluateInJSThreadBlocking {
        when {
            ...
            descriptor.kind is StructureKind.LIST -> JSIArrayListDecoder(
                format,
                currentValue,
                currentValue.asObject(runtime).asArray(runtime),
            )
...
internal class JSIValueDecoder(
    format: JSIFormat,
    value: Value,
) : AbstractJSIValueDecoder(format, value)

internal class JSIObjectMapDecoder(
    override val format: JSIFormat,
    override val value: Value,
    private val jsiObject: Object,
) : AbstractRuntimeObjectMapDecoder<Value>(),
    NodeDecoder by JSIValueDecoder(format, value) {
    ...
    ): JSIValueDecoder = when (index % 2 == 0) {
...
internal class JSIArrayListDecoder(
    override val format: JSIFormat,
    override val value: Value,
    ...
    override fun getElementAtIndex(index: Int): Value = runtime.evaluateInJSThreadBlocking {
        array.getValueAtIndex(runtime, getKeyAtIndex(index))
    }
    ...
    override fun <T> buildDecoderForSerializableElement(
        ...
        index: Int,
        deserializer: DeserializationStrategy<T>,
    ): JSIValueDecoder = JSIValueDecoder(format, decodeElement(descriptor, index))
...
internal class JSIObjectClassDecoder(
    override val format: JSIFormat,
    override val value: Value,
    private val jsiObject: Object,
...
internal class JSISealedClassDecoder(
    override val format: JSIFormat,
    override val value: Value,
    private val jsiObject: Object,
) : AbstractRuntimeObjectClassDecoder<Value>(),
    ...
    override fun getElementAtIndex(index: Int): Value =
        throw JSIDecodingException("JSISealedClassDecoder should not be used to decode any elements")
    ...
    override fun <T> buildDecoderForSerializableElement(
        ...
        index: Int,
        deserializer: DeserializationStrategy<T>,
    ): JSIValueDecoder = JSIValueDecoder(format, value)
...
internal class JSIObjectInstanceDecoder(
    override val format: JSIFormat,
    override val value: Value,
) : AbstractRuntimeObjectClassDecoder<Value>(),
    NodeDecoder by JSIValueDecoder(format, value) {
