...
internal open class GraalNode(
    override val graalObject: Value,
    ...
    override fun getObject(key: String): Node? = graalObject
        .lockIfDefined {
    ...
    override operator fun get(key: String): Any? = graalObject
        .lockIfDefined {
            getMember(key)
        }?.handleValue(format)

    override fun <R> getInvokable(key: String, deserializationStrategy: DeserializationStrategy<R>): Invokable<R>? = graalObject
        .lockIfDefined {
            graalObject.getMember(key)
        }?.toInvokable(format, deserializationStrategy)

    override fun <R> getFunction(key: String): Invokable<R>? = graalObject
        .lockIfDefined {
            graalObject.getMember(key)
        }?.toInvokable(format, null)

    override fun getList(key: String): List<*>? = graalObject
        .lockIfDefined {
            graalObject.getMember(key)
        }?.toList(format)

    override fun <T> getSerializable(key: String, deserializer: DeserializationStrategy<T>): T? = graalObject.blockingLock {
        key
            .takeIf(graalObject::hasMember)
