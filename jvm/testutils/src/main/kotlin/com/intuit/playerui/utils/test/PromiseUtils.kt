...
public interface PromiseUtils {
    ...
    public fun assertChain(chain: List<Any?>, vararg expected: Any?): Unit = Assertions.assertEquals(expected.asList(), chain)
    ...
    public fun <T : Any> Promise.thenRecord(deserializer: DeserializationStrategy<T>, list: MutableList<Any?>): Promise =
        then(deserializer) {
            list.add(it)
        }
